...
The MCP migration supplies #87's stateless opaque `readHandle` guard and structural-fingerprint state. HTTP calls to this tool must require the validated handle; `expectedRevisionId` is accepted only as the migration-defined matching assertion, never as authorization. It must not recreate session-keyed tracker behavior. #105 then exposes the public structural index used for range discovery, and #106 establishes the compatible markdown and workspace contract.
...
- **Reuse the migrated guard/WriteControl chain** exactly as the post-migration `replaceDocumentWithMarkdown` flow: validate the supplied opaque handle, use its stored revision as `requiredRevisionId`, and compare any supplied `expectedRevisionId` to that record. #108 may later add range precision and re-resolution; this tool remains safe under the document-scoped contract until then.
...
After migration (opaque read-handle guard), #105 (public structural index), and **#106** (so exported markdown round-trips and the editable-copy contract is safe). Alongside #88 (shared `dryRun`; `batchModifyText` for many small *text* edits, this tool for *structured* content). #108 can later add range precision/re-resolution. Shared follow-up with #88: transactional single-batchUpdate replace.
