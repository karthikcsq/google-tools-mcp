...
The MCP migration owns the SDK v2 request runtime and changes `context.log` from protocol notifications to server-side stderr/file routing. #91 does not reimplement that transport behavior. It remains open for the persistent redacted JSONL sink, per-executed-tool record shape, rotation, aggregation, feedback opt-in, startup timing, and runbook. Instrument the migration's final façade/choke point rather than the removed fastmcp wrapper, and re-baseline line anchors before implementation.
...
After migration, the enabling fact is the final registration façade: one choke point wraps every tool's **execution**, with tool name and a try/catch in scope, covering category tools, utilities, and legacy aliases. Re-baseline the final hook and preserve the alias single-record invariant. There is no session key in the stateless protocol.
...
- **Record shape, values never serialized:** `ts`, `event:'tool_call'`, `tool`, `reqId` (monotonic), `durationMs`, `outcome:'ok'|'user_error'|'error'`, `errCode`, `errMsg` (redacted, truncated), `argShape` (per-arg type + length/size only). Scope honesty: this instruments **executed** tool calls; validation and unknown-method failures that the SDK rejects before `execute` produce no record. Acceptance wording reflects that; instrumenting the SDK dispatch boundary is a follow-up, not smuggled into this plan.
...
- **Runbook as a docs page:** `docs/troubleshooting-runbook.md` — log locations per platform, the JSONL schema (with a test asserting the documented field list equals the emitted field set), auth vs API vs internal failure signatures, and correlation via `reqId`. Linked from README troubleshooting.
...
2. The migration's final registration façade/choke point: timer + outcome classification + `argShape` + `logToolCall`. Keep the existing alias double-wrap invariant, but do not patch a removed fastmcp `addTool` path. (~20 lines after re-baselining.)
...
6. `mapsClient.redactSecrets` → delegates to shared redactor (keeping value-registration); register the #75 persistent HTTP token through the final runtime's secret path when HTTP lifecycle work lands.
...
After the MCP migration and #82 (config file must be able to set the log/JSONL paths and level). Re-baseline the façade instrumentation point after migration; coordinate its small registration edit with #71 rather than assuming `tools/index.js` remains the runtime boundary.
