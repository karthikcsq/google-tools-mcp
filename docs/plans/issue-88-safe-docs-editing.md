...
- **`listHeadings` as a standalone tool** — same extraction exposed: `listHeadings(documentId, tabId?)`. Promote `getHeadingLevel` out of the converter; handle TITLE/SUBTITLE/HEADING_N, empty bodies (empty array), nullable `headingId` (return null, don't fabricate).
...
1. `dist/tools/docs/batchModifyText.js`: schema `operations` (1–50 ops; request-count cap enforced post-build at 400), shared `tabId`, `dryRun`; snapshot fetch → snapshot-based target resolution → overlap check → descending sort → one guarded `batchUpdate`. Register in `docs/index.js`.
2. `findTextRangeInDoc` refactor in `googleDocsApiHelpers.js`.
3. `dist/tools/docs/listHeadings.js` + shared heading-extraction helper.
...
- `listHeadings`: TITLE/SUBTITLE/H1–H3, headingId-less heading → null, tabs variant, empty doc.
- Heading map: post-write map matches the narrow-mask fixture, tab variant included.
...
- Structure questions cost one narrow-mask call.
