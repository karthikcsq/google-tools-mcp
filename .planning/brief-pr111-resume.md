# NOTE: a previous run of this exact brief was killed mid-flight. The worktree has UNCOMMITTED partial work in dist/mime.js and tests/mime.test.js from that run. Review that diff first (git diff), keep what is correct, and continue from there. Do not discard it blindly.
