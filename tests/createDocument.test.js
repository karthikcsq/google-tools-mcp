...
// createDocument also re-reads the finished document and converts it back to
// markdown to seed post-create read state (#87 gap 2), so this module mock has
// to expose docsJsonToMarkdown too or the import fails to link.
const docsJsonToMarkdown = jest.fn(() => '# Roadmap');
...
function setSuccessfulClients() {
    // documents.get / files.get back the post-create read-state seeding step
    // (#87 gap 2). Without them that step fails and adds a warning of its own,
    // which would mask what these tests are actually asserting about content
    // warnings.
    fakeDrive = {
        files: {
            create: jest.fn(async () => ({ data: createdDocument })),
    ...
    fakeDocs = {
        documents: {
            ...
            get: jest.fn(async () => ({ data: { documentId: 'doc-123', revisionId: 'rev-1', body: { content: [] } } })),
        },
    };
    insertMarkdown.mockReset().mockResolvedValue({ warnings: [] });
    formatInsertResult.mockClear();
    docsJsonToMarkdown.mockClear();
}
