...
// persistTokenCredentials writes token.json atomically: open(tmp, 'wx', 0o600)
// -> handle.writeFile -> sync -> close -> rename -> chmod, with the tmp file
// unlinked in a finally. All of those have to exist on this mock, and the
// payload arrives through the file handle rather than a bare writeFile, so the
// handle records into writeFileCalls as [path, payload, encoding] to keep the
// assertions below reading the saved token the same way.
jest.unstable_mockModule('fs/promises', () => ({
    ...
    rename: (...args) => { renameCalls.push(args); return Promise.resolve(); },
    open: async (filePath) => ({
        writeFile: (payload, encoding) => { writeFileCalls.push([filePath, payload, encoding]); return Promise.resolve(); },
...
describe('auth.js interactive OAuth consent flow (issue #115)', () => {
    ...
    it('saves credentials and succeeds on a normal exchange that returns a refresh token', async () => {
        ...
        // The atomic write finished: the temp file was renamed into token.json.
        expect(renameCalls).toHaveLength(1);
        expect(String(renameCalls[0][1])).toMatch(/token\.json$/);
